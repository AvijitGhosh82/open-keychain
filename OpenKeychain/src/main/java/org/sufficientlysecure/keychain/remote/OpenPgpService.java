...
public class OpenPgpService extends Service {
    ...
    private KeyIdResult returnKeyIdsFromEmails(Intent data, String[] encryptionUserIds) {
        ...
        if (noUserIdsCheck || missingUserIdsCheck || duplicateUserIdsCheck) {
            ...
            long[] keyIdsArray = getUnboxedArray(keyIds);

            Intent resultPendingIntent = new Intent(getBaseContext(), RemoteServiceActivity.class);
            resultPendingIntent.setAction(RemoteServiceActivity.ACTION_SELECT_PUB_KEYS);
            resultPendingIntent.putExtra(RemoteServiceActivity.EXTRA_SELECTED_MASTER_KEY_IDS, keyIdsArray);
            resultPendingIntent.putExtra(RemoteServiceActivity.EXTRA_NO_USER_IDS_CHECK, noUserIdsCheck);
            resultPendingIntent.putExtra(RemoteServiceActivity.EXTRA_MISSING_EMAILS, missingEmails);
            resultPendingIntent.putExtra(RemoteServiceActivity.EXTRA_DUPLICATE_EMAILS, duplicateEmails);
            resultPendingIntent.putExtra(RemoteServiceActivity.EXTRA_DATA, data);
    ...
    private Intent encryptAndSignImpl(Intent data, InputStream inputStream,
        ...
        try {
            ...
            long[] keyIds;
            {
                ArrayList<Long> encryptKeyIds = new ArrayList<>();
                ...
                // add user ids from references
                if (data.hasExtra(OpenPgpApi.EXTRA_KEY_REFERENCES)) {
                    // get key ids based on given user ids
                    String[] keyReferences = data.getStringArrayExtra(OpenPgpApi.EXTRA_KEY_REFERENCES);
                    data.removeExtra(OpenPgpApi.EXTRA_KEY_REFERENCES);
                    // give params through to activity...
                    KeyIdResult result = returnKeyIdsFromEmails(data, keyReferences);
                ...
                keyIds = getUnboxedArray(encryptKeyIds);
            }
    ...
    private Intent getKeyIdsImpl(Intent data) {
        ...
        if (data.hasExtra(OpenPgpApi.EXTRA_KEY_IDS)) {
            ...
            long[] keyIds = getUnboxedArray(keyResult.mKeyIds);
    ...
    @NonNull
    private static long[] getUnboxedArray(@NonNull ArrayList<Long> arrayList) {
